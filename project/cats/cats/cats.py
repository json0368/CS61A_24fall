...
def about(subject):
    ...
    def select(paragraphs):
        ...
        for word in subject:
            for paragraph_word in split(paragraphs):
                if lower(word) == lower(paragraph_word):
                    return True              
        return False
    return select
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    corret = 0
    all = len(typed_words)
    lenght = min(len(typed_words),len(source_words))
    for i in range(0,lenght):
        if typed_words[i] == source_words[i]:
            corret += 1
    return 100 * corret / all
    # END PROBLEM 3
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    progress /= len(source)
    user_imformation = {'id' : user_id, 'progress' : progress}
    upload(user_imformation)
    return progress
    # END PROBLEM 8
